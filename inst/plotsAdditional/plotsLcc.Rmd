...
params:
    ...
    path: "C:/Users/ricardar/Documents/Results/BRICK/adaptHpIntangCost/calibration-2015-CP_2026-03-26_17.09.44/"
    scenNames: NULL
    name: ""
    region: "DEU"
---
...
```{r Plot renovationLCOH, mixed lt by time and hs}
showAnalysisPlot("line", data %>% filter(vin == "1980-1989"), "renLcohMixedFull",
                 xname = "ttotIn", yname = "value", color = "hsr", rprt = c("hs", "typ", "loc"),
                 sumUp = "costType",
                 facets = NULL, xlabName = "Time of installation", ylabName = "Costs in US Cents/kWh",
                 tmpl = tmpl, linetype = "hsr")
```

```{r Plot renovation LCOH, mixed lt by time, hs and cost type}
showAnalysisPlot("bar", data %>% filter(vin == "1980-1989"), "renLcohMixedFull",
                 xname = "ttotIn", yname = "value", color = "costType", rprt = c("hs", "typ", "loc"),
...
renInRatioVar <- unique(sub("Full$", "", grep("renInRatio", data$variable, value = TRUE)))
renLccDiffVar <- unique(sub("Full$", "", grep("renLccDiff", data$variable, value = TRUE)))
hsRef <- sub("renInRatio(.*)$", "\\1", renInRatioVar)
...
renInRatioVarFull <- paste0(renInRatioVar, "Full")
renLccDiffVarFull <- paste0(renLccDiffVar, "Full")
